...
<table border="0" cellpadding="0" cellspacing="0" width="100%" class="scrollTable">
				 ...
				 @foreach($team_edit_list as $dd)
					           ...
					           @if($dd->player_role=='Bowler')	
							<tr class="select_tick <?php if(in_array($dd->player_id,$user_team_sa)){echo "active"; } else { echo "inactive"; } ?>" id="bowler_id{{$dd->player_id}}" onclick="select_tick(this,'bowler');" >	
						...
						<td width="100px">{{$dd->credit_point}}<span class="tick" id="bowid_bow{{$dd->player_id}}" <?php if(in_array($dd->player_id,$user_team_sa)){  } else { echo 'style="display:none"'; } ?> ><img  src="{{url('public/site/image/tick.png')}}"> </span></td>				
						</tr>								
						@endif								
						@endforeach 
								</tbody>
