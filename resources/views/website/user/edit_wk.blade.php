	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%" class="scrollTable">
				 ...
				 @foreach($team_edit_list as $dd)				
			                ...
			                @if($dd->player_role=='Wicketkeeper batsman' || $dd->player_role=='Wicketkeeper')	
									<tr class="select_tick <?php if(in_array($dd->player_id,$user_team_sa)){echo "active"; } else { echo "inactive"; } ?>" id="wktick_ide{{$dd->player_id}}"  onclick="select_tick(this,'wicketkeeper');">
								<td width="60px">
								<img  src="{{url('public/site/image/wk_icon.png')}}"></td>
								...
								<td width="100px">{{$dd->credit_point}}<span class="tick" id="wkid_wk{{$dd->player_id}}" <?php if(in_array($dd->player_id,$user_team_sa)){  } else { echo 'style="display:none"'; } ?> id="wkid_wke{{$dd->player_id}}" ><img  src="{{url('public/site/image/tick.png')}}"> </span></td>		
								</tr>						
							@endif			
								@endforeach  
								</tbody>
