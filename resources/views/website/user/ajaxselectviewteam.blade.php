...
<?php //Session::put('unique_id',$unique_id); ?>
					 <?php $fetch = DB::table('matches')
					                ->where('unique_id', '=', Session::get('unique_id'))
									->first();
...
<div class="create">
<a class="btn btn-primary btn-sm" href="{{url('edit-team/'.base64_encode($team_players->id).'/'.base64_encode($team_players->match_id).'/'.base64_encode($team_players->team_no))}}">Edit Team {{$team}}</a>
</div>
@endif

@if (date('Y-m-dH:i:s') > $con) 
						<a href="{{url('play-pts',Session::get('unique_id'))}}" class="btn btn-large btn-info" >Individual Player Points</a>
				@endif
<div class="team_bottom">

			<?php $unique_id=Session::get('unique_id');?>	
