  <!--<div class="modal-body drpnselected" style="height: 125px;float:left;width: 50%;">-->
													<button class="btn dropdown-toggle btn-select" id="substitute_player" data-toggle="dropdown" href="#" style="border: 3px solid #b3b3b2;border-radius:0px;" >Select Substitute <span class="caret"></span></button>
											...
											@if($players->player_role=='Opening batsman' || $players->player_role=='Batsman' || $players->player_role=='Top-order batsman' || $players->player_role=='Middle-order batsman')
												...
												<span id="view_player_name_batting_nselected1"> {{$players->player_name}}</span>/ <span>{{$players->credit_point}}</span>/ <span id="view_player_team_name_batting_nselected1">{{$players->player_team_name}}</span>
												</li>
											 ...
											 @elseif($players->player_role=='No')	
												...
												<input type="hidden" class="credit_point_subs" value="{{$players->credit_point}}">
												<span id="view_player_name_bowler_nselected1"> {{$players->player_name}}</span>/ <span>{{$players->credit_point}}</span> / <span id="view_player_team_name_bowler_nselected1">{{$players->player_team_name}}</span>
												</li>
												...
												<input type="hidden" class="credit_point_subs" value="{{$players->credit_point}}">
												<span id="view_player_name_allrounder_nselected1">{{$players->player_name}}</span> / <span>{{$players->credit_point}}</span>/ <span id="view_player_team_name_allrounder_nselected1">{{$players->player_team_name}}</span>
												</li>
												...
												<input type="hidden" class="credit_point_subs" value="{{$players->credit_point}}">
												<span id="view_player_name_wicketkeeper_nselected1">{{$players->player_name}}</span> / <span>{{$players->credit_point}}</span>/ <span id="view_player_team_name_wicketkeeper_nselected1">{{$players->player_team_name}}</span>
												</li>
												@endif
									...
									@if($user_create_team->substitute)
	
@php($substitute=DB::table('fantasy_team_players')->whereplayer_id($user_create_team->substitute)->wherematch_id($user_create_team->match_id)->first())
		<script>
				
					$( document ).ready(function() {
    $("#substitute_player li").parents('.drpnselected').find('.dropdown-toggle').html('<input type="hidden" name="selected_substitute_player_id" id="selected_substitute_player_id" value="<?php echo $user_create_team->substitute; ?>"><input type="hidden" name="selected_credit_point" id="selected_credit_point" value="<?php echo $substitute->credit_point; ?>"><?php echo $substitute->player_name; ?> <span class="caret"></span>');
});
