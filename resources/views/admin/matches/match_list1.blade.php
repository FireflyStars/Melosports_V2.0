...
@php($api=$pay_apikey->cric_api_key)
                <tbody>
                <?php   $cricketMatchesTxt = file_get_contents('http://cricapi.com/api/matches/?apikey='.$api.'&v=2');  // change with your API key
    $cricketMatches = json_decode($cricketMatchesTxt); 


    ?>
       @if (isset($cricketMatches->matches))
                        @foreach ($cricketMatches->matches as $match_list)
                        
                          <tr>
                                <td>{{ $match_list->unique_id }}</td>
                                <td>{{ $match_list->dateTimeGMT }}</td>
                                <td><?php echo $match_list->{'team-1'} ; ?></td>
                                <td><?php echo $match_list->{'team-2'} ; ?></td>
                                <td><?php echo $match_list->{'type'} ; ?></td>
                                @if(array_key_exists('matchStarted',$match_list))
                                <td>@if($match_list->matchStarted=='true') YES @else NO @endif  </td>
                                
                            @else
                         <td> Yes  </td>
                            @endif
                            <td>@if($match_list->squad=='true') YES @else NO @endif  </td>
                                <td>
                                 @can('matches_view')
                                 ...
                                 <input type="hidden" value="<?php echo $match_list->{'team-1'} ; ?>" name="team_1">
                                 <input type="hidden" value="<?php echo $match_list->{'team-2'} ; ?>" name="team_2">
