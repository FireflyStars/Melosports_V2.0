...
use DB;
use App\Match;use App\SocialSetting;
class DemoCron extends Command
{
	...
	//	echo $today;
		...
		$pay_apikey=SocialSetting::findorfail(1);$apikey=$pay_apikey->cric_api_key;
...
foreach($matches as	$matchlist)
{
    ...
    $cricketMatchesTxt=file_get_contents('http://apizipper.com/api/fantasy_summary/?apikey='.$apikey.'&&unique_id='.$matchlist->unique_id);
...
foreach ($fielding->scores as $scoresfield)
	 ...
	 {	 
		...
		$runout= $scoresfield->runout * $point_table->run_out;
		$field_total=$catch+$stumping+$runout;
...
$fielding_array['runout'] = array($runout);
