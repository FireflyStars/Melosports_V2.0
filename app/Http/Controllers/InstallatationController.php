...
class InstallatationController extends Controller
{
    ...
    public function installProject(Request $request)
    {
       ...
       if($request->host_name == null || $request->database_name == null || $request->user_name == null){
	...
	function createTables(Request $request)
	{
      ...
      $base1 .= '://'.$_SERVER['HTTP_HOST'] . str_replace(basename($_SERVER['SCRIPT_NAME']),"",$_SERVER['SCRIPT_NAME']);
       ...
       $data_path   =  $base1."public/database/install.sql";
       // dd($data_path);
			 $query = $this->get_content($data_path);
       // dd($query);
